# Kater Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/kater.svg?label=pypi%20(stable))](https://pypi.org/project/kater/)

The Kater Python library provides convenient access to the Kater REST API from any Python 3.9+
...
pip install git+ssh://git@github.com/kater-ai/kater-python-sdk.git
```
...
```sh
# install from the production repo
pip install 'kater[aiohttp] @ git+ssh://git@github.com/kater-ai/kater-python-sdk.git'
```
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `kater.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `kater.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `kater.APIError`.
...
client.with_options(max_retries=5).v1.api_keys.list()
```
...
client.with_options(timeout=5.0).v1.api_keys.list()
```
...
You can enable logging by setting the environment variable `KATER_LOG` to `info`.

```shell
$ export KATER_LOG=info
```
...
api_key = response.parse()  # get the object that `v1.api_keys.list()` would have returned
print(api_key)
```
...
```python
import httpx
from kater import Kater, DefaultHttpxClient

client = Kater(
    # Or use the `KATER_BASE_URL` env var
